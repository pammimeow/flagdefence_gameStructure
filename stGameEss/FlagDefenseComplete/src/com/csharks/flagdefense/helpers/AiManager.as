package com.csharks.flagdefense.helpers
{
	...
	public class AiManager
	{
		 ...
		 * 
		 */
		public function AiManager(_greenSoldiers:Vector.<Soldier>,_blueSoldiers:Vector.<Soldier>,_greenBombs:Vector.<Bomb>,_blueBombs:Vector.<Bomb>,_explosions:Vector.<Explosion>,_tileWidth:Number,_screenOffset:Point)
		{
		...
		public function makeDecision(value:uint):uint
		{
			...
			if(normalMove){
				...
				/*
				character=findAiSoldier();
				character.path.splice(0,character.path.length);
				getUpdatedCollisionArray(true);
				realtimeCollision[character.paintPoint.y][character.paintPoint.x]=0;
				character.path=PathFinder.go(character.paintPoint.x,character.paintPoint.y,greenFlagPt.x,greenFlagPt.y,realtimeCollision);
				character.path.reverse();
				character.path.splice(value+1);//trace("out",character.path);
		...
		public function set groundLayer(value:GroundLayer):void
		{
			_groundLayer = value;
		}
